...
export function SoftwareEngineerElite({ data, customization }: TemplateProps) {
  ...
  return (
    <div className="flex flex-col h-full w-full text-[11px] leading-[1.6] text-text-primary bg-surface" style={{ fontFamily: 'var(--font-mono), monospace' }}>
      {/* Header Area */}
      <header className="w-full px-8 pt-8 pb-6 border-b-2" style={{ borderColor: accentColor }}>
        <div className="flex justify-between items-end">
          ...
          <div className="flex gap-4 text-xs font-semibold text-text-muted pb-1">
        ...
        <div className="w-[65%] p-8 flex flex-col gap-6 border-r border-gray-200">
          {summary && (
            <section>
              ...
              <div className="text-text-secondary font-sans text-[12px]" dangerouslySetInnerHTML={{ __html: summary }} />
            </section>
          )}
                ...
                {experience.map(exp => (
                  <div key={exp.id} className="relative pl-4 border-l-2" style={{ borderColor: `${accentColor}30` }}>
                    ...
                    <div className="flex justify-between items-baseline mb-1">
                      <h4 className="font-bold text-[13px] text-text-primary font-sans">{exp.position}</h4>
                    ...
                    <div className="text-[12px] font-bold mb-2 text-text-secondary">
                      {exp.company}
                    </div>
                    <ul className="list-disc list-inside flex flex-col gap-1 text-text-secondary font-sans pl-1">
                    ...
                    {exp.technologies?.length > 0 && (
                      <div className="mt-2 flex flex-wrap gap-1">
                        {exp.technologies.map((tech, i) => (
                          <span key={i} className="text-[9px] px-1.5 py-0.5 border border-gray-200 rounded text-text-muted font-bold">{tech}</span>
                        ))}
                      </div>
                    )}
                  </div>
              ...
              {skills?.languages?.length > 0 && (
                <div className="bg-gray-50 rounded-md p-3 border border-gray-100">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h4 className="font-bold text-[11px] text-text-primary uppercase tracking-wide">Programming Languages</h4>
                  </div>
              ...
              {skills?.technical?.length > 0 && (
                <div className="bg-gray-50 rounded-md p-3 border border-gray-100">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h4 className="font-bold text-[11px] text-text-primary uppercase tracking-wide">Core Technologies</h4>
                  </div>
                  <p className="text-[11px] text-text-muted leading-relaxed font-mono">
              ...
              {skills?.frameworks?.length > 0 && (
                <div className="bg-gray-50 rounded-md p-3 border border-gray-100">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h4 className="font-bold text-[11px] text-text-primary uppercase tracking-wide">Frameworks & Libraries</h4>
                  </div>
                  <p className="text-[11px] text-text-muted leading-relaxed font-mono">
              ...
              {skills?.tools?.length > 0 && (
                <div className="bg-gray-50 rounded-md p-3 border border-gray-100">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h4 className="font-bold text-[11px] text-text-primary uppercase tracking-wide">Developer Tools</h4>
                  </div>
                  <p className="text-[11px] text-text-muted leading-relaxed font-mono">
                    {skills.tools.join(' • ')}
                  </p>
              ...
              {skills?.platforms?.length > 0 && (
                <div className="bg-gray-50 rounded-md p-3 border border-gray-100">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h4 className="font-bold text-[11px] text-text-primary uppercase tracking-wide">Cloud & Infrastructure</h4>
                  </div>
                  <p className="text-[11px] text-text-muted leading-relaxed font-mono">
          ...
          {projects?.length > 0 && (
            <section>
              ...
              <div className="flex flex-col gap-4">
                {projects.map(proj => (
                  <div key={proj.id} className="bg-surface p-3 border border-gray-200 rounded-lg shadow-sm">
                    <h4 className="font-bold text-[12px] text-text-primary mb-1">{proj.name}</h4>
                    <p className="text-text-muted font-sans text-[10.5px] mb-2 leading-snug">{proj.description}</p>
          ...
          {education?.length > 0 && (
            <section>
              ...
              <div className="flex flex-col gap-3 font-sans">
                {education.map(edu => (
                  <div key={edu.id}>
                    <div className="font-bold text-[11px] text-text-primary">{edu.degree}</div>
                    <div className="text-text-muted text-[10.5px]">{edu.institution}</div>
                    <div className="text-text-muted text-[10px] font-mono mt-0.5">{formatMonthYear(edu.graduationDate)}</div>
                  </div>
                ))}
              </div>
            </section>
